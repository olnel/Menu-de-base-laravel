...
const update = (rowData) => {
    router.visit(route('remorquephoto.show', {remorquephoto: rowData.id}), {
        preserveState: true,
        preserveScroll: true,
        onSuccess: (reponse) => {
...
const savePhoto = () => {
    ...
    const url = photo.id
        ? route('remorquephoto.update', photo.id)
        : route('remorquephoto.store');
...
const removePhoto = async () => {
    ...
    if (photo.id) {
        confirm_delete(() => {
            router.delete(route('remorquephoto.destroy', photo.id), {
                preserveScroll: true,
                onSuccess: () => {
    ...
    >
        <div class="space-y-6 mt-4">
            ...
            <a-row :gutter="[16, 0]">
                ...
                <a-col :xs="24">
                    <FormItem required label="Etat Remorque" class="mb-4">
                        <a-textarea

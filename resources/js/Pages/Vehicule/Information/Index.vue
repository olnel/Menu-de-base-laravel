...
import ListePhotos from "@/Pages/Vehicule/Information/ListePhotos.vue";
import VehiculeElement from "@/Pages/Vehicule/VehiculeForm/VehiculeElement.vue";
...
const title = ref('Informations sur le véhicule: ');
const ref_photo = ref();
...
const localRemorque = ref([...props.liste_remorque]);
...
watch(() => props.liste_remorque, (newVal) => {
...
const applyFilters = (data) => {
    ...
    const url = route('vehicule.info', {vehicule: props.info.data.vehicule.id});
...
<template>
    <SousMenuPrincipale :title="title + info.data.vehicule.immatriculation" selectedMenu="vehicule" :backTo="route('vehicule.index')">
        <a-card size="middle" class="h-full">

            <a-tabs v-model:activeKey="activeKey" animated class="overflow-hidden">
                <a-tab-pane key="1" tab="Informations du vehicule" v-if="can('vehicule.update')">
                    <VehiculeInformation :information="info.data.vehicule"
                ...
                <a-tab-pane key="2" tab="Eléments Véhicule" v-if="can('vehicule.index_element_vehicule')">
                    ...
                    <VehiculeElement :tab_element_vehicule="localElements"

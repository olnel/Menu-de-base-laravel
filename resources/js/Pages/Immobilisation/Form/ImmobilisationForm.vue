...
const add = () => {
    title.value = "Immobilisation";
    open.value = true;
};

const update = (rowData) => {

    router.visit(`${route('immobilisation.show', {immobilisation: rowData.id})}`, {
        preserveState: true,
        preserveScroll: true,
        only: ['flash'],
        onSuccess: (reponse) => {
...
const submit = () => {
    ...
    const url = form.id ? route('immobilisation.update', form.id) : route('immobilisation.store');
    form.transform(data => ({
        ...data,
...
<template>
    ...
    >
        ...
        <div class="">
            ...
            <form-item required has-feedback label="Valeur" :help="form.errors.valeur">

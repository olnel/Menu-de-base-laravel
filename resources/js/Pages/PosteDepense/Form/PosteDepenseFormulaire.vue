...
const add = () => {
    title.value = "Nouveau Poste de dépense";
    open.value = true;
};

const update = (rowData) => {

    router.visit(`${route('postedepense.show', {postedepense: rowData.id})}`, {
        preserveState: true,
        preserveScroll: true,
        only: ['flash'],
        onSuccess: (reponse) => {
...
const submit = () => {
    ...
    const url = form.id ? route('postedepense.update', form.id) : route('postedepense.store');
    form.transform(data => ({
        ...data,

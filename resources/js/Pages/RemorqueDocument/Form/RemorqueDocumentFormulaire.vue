...
const titre = ref();
const localimmatriculation = ref(null);
const element = ref();
const localDocument = ref({...props.vehiculeDocuments});
const localVehiculeListe = ref([...props.vehiuclesListe]);
const dateFormat = 'YYYY/MM/DD';

const openModal = ( isNew = true) => {
...
const update = (rowData) => {
    console.log(rowData.immatriculation)
    router.visit(route('vehiculedocument.show', {vehiculedocument: rowData.id}), {
        preserveState: true,
        preserveScroll: true,
        onSuccess: (reponse) => {
            ...
            showModal.value = true;
            localimmatriculation.value = rowData.immatriculation;
...
const removeDocument = async () => {
    ...
    if (localDocument.value.id) {
        confirm_delete(() => {
            router.delete(route('vehiculedocument.destroy', localDocument.value.id), {
                preserveScroll: true,
                onSuccess: () => {
                    localDocument.value = {};
...
const saveDocument = () => {
    ...
    formData.append('immatriculation', document.immatriculation);
    ...
    router.post(url, formData, {
        ...
        onError: (errors) => {
            message.error('Erreur lors de l\'enregistrement du document');
...
<template>
    ...
    >
        <div class="space-y-6 mt-4">
            ...
            <a-row :gutter="[16, 0]">
                <a-col :xs="24" :lg="24">
                    <FormItem required label="Véhicule">

                        <autocomplete-component :options="localVehiculeListe"
                                                v-model="localDocument.immatriculation"
                                                class=" !w-full"
                                                size="large"
                                                :field-config="{
                                                        label: 'label',

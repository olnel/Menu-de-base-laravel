                            ...
                            <a-col :xs="24" :lg="12">
                                ...
                                >
                                    <a-select
                                        ...
                                        size="large"
                                        placeholder="Aucun véhicule associé"
                                        class="w-full"
                                        disabled
                                    >
                                        <a-select-option
                                ...
                                <form-item label="Remorque Associée">
                                    <a-input
                                        :value="selectedRemorque?.numero_remorque || '-'"
                                        size="large"
                                        class="w-full"
                                        disabled
                                    />
...
const emits = defineEmits(["transaction-saved"]);
...
watch(
    ...
    (newChauffeurId) => {
        ...
        const chauffeur = props.chauffeurs.find((c) => c.value === newChauffeurId);
        if (chauffeur?.vehicule_id) {
            form.vehicule_id = chauffeur.vehicule_id;
        }
    }
);

const selectedRemorque = computed(() => {
    if (!form.vehicule_id) return null;
    return props.vehicules.find((v) => v.id === form.vehicule_id)?.remorque ?? null;
});
...
const add = (options = {}) => {
    ...
    form.reset();
    form.clearErrors();
    form.voyage_id = options.voyage_id || null;
    form.vehicule_id = options.vehicule_id || null;
    form.chauffeur_id = options.chauffeur_id || null;
...
const update = (record) => {
    ...
    form.voyage_id = record.voyage_id;
